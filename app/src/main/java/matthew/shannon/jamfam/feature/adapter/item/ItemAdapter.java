...
public class ItemAdapter extends RecyclerView.Adapter {
    ...
    public ItemAdapter(Map<Integer, ItemViewHolderFactory> viewHolderFactories, List<?> items, FlowService flow) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ((ItemViewHolder) holder).bind(items.get(position), flow);
    }
    ...
    public void updateList(List<?> newItems) {
        DiffUtil.DiffResult diffResult = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                ...
                boolean ret = false;
                if (oldItem instanceof User && newItem instanceof User) ret = ((User) oldItem).get_id().equals(((User) newItem).get_id());
                else if (oldItem instanceof Track && newItem instanceof Track) ret = ((Track) oldItem).get_id().equals(((Track) newItem).get_id());
                else if (oldItem instanceof Settings && newItem instanceof Settings) ret = ((Settings) oldItem).getValue().equals(((Settings) newItem).getValue());
                return ret;
            }
